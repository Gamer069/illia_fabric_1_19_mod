...
public class DrinkingC2SPacket {
	...
	public static void receive(MinecraftServer server, ServerPlayerEntity player, ServerPlayNetworkHandler handler, PacketByteBuf buf, PacketSender responseSender) {
		...
		if (isAroundWater(player, world, 2)) {
			...
			player.sendMessage(Text.literal("Thirst: " + ((IEntityDataSaver) player).getPersistentData().getInt("thirst")).fillStyle(Style.EMPTY.withColor(Formatting.GOLD)));
		} else {
		...
		if (isAroundWaterCauldron(player, world, 2)) {
			...
			world.playSound(null, player.getBlockPos(), SoundEvents.ENTITY_GENERIC_DRINK, SoundCategory.NEUTRAL, 0.5F, world.random.nextFloat() * 0.1F + 0.9F);
			ThirstData.addThirst(((IEntityDataSaver) player), 1);
			player.sendMessage(Text.literal("Thirst: " + ((IEntityDataSaver) player).getPersistentData().getInt("thirst")).fillStyle(Style.EMPTY.withColor(Formatting.GOLD)), true);
		} else {
			player.sendMessage(Text.translatable(MESSAGE_WATER_ERROR)
				.fillStyle(Style.EMPTY.withColor(Formatting.DARK_PURPLE)), false);
			player.sendMessage(Text.literal("Thirst: " + ((IEntityDataSaver) player).getPersistentData().getInt("thirst")).fillStyle(Style.EMPTY.withColor(Formatting.DARK_PURPLE)), true);
